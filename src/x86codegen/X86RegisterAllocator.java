...
public class X86RegisterAllocator {
  ...
  private LinkedList<Object> allocatedRegs;
  private LinkedList<Object> spilledRegs;
  private int[] regMap;
  ...
  private void determineAllocationOrderAndSpills(Function func) {
    ...
    while (numConflictingRegs > 0) {
        ...
        for (int i = 0; i <= maxRegNum; i++) {
          if (isUndecidedReg[i]) {
            if (numConflicts[i] < availableRegs) {
              allocatedRegs.addFirst((Integer)(i));
      ...
      if (numConflictingRegs > 0) {
        ...
        spilledRegs.add((Integer)(mostConstrainedReg));
    ...
    Iterator<Object> allocateIterator = allocatedRegs.iterator();
    ...
    Iterator<Object> spillIterator = spilledRegs.iterator();
  ...
  private void assignPhysicalRegs(Function func) {
    ...
    Iterator<Object> listIter = allocatedRegs.iterator();
...
//        }
        ...
        if (fudgeFactor > 0) {
          for (Operation currOper = subsequentOper; currOper != null;
            ...
            currOper.getSrcOperand(1).setValue((Integer)(oldOffset + fudgeFactor));
          }
        }
      }
